...
export default function LoginPage() {
  ...
  const validate = () => {
    let valid = true
    if (!username.trim()) {
      showFieldError('username', '请输入学号或工号')
      valid = false
    }
    if (!password) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate(user.force_password_change ? '/change-password' : `/${user.role}`)
    } catch (err: unknown) {
      setLoading(false)
        ...
        <div className="login-layout" style={styles.layout}>
          ...
          <section style={styles.card}>
            <div style={{ textAlign: 'center', marginBottom: 28 }}>
              ...
              <h1 style={{ color: '#fff', fontSize: 26, lineHeight: 1.2, margin: '0 0 8px', fontWeight: 800 }}>欢迎回来</h1>
              <p style={{ color: 'rgba(232,245,251,.62)', fontSize: 14, margin: 0 }}>登录账号，继续探索课程知识</p>
            </div>
            ...
            <form onSubmit={handleSubmit} noValidate>
              ...
              <div className="login-field" style={{ marginBottom: 16 }}>
                <label style={{ display: 'block', color: 'rgba(232,245,251,.76)', fontSize: 13, fontWeight: 600, marginBottom: 7 }}>学号 / 工号</label>
                <div className={`login-input-wrap${shakeField === 'username' ? ' login-shake' : ''}`} style={inputWrapperStyle('username')}>
                  ...
                  <input
                    ...
                    onBlur={() => { if (!username.trim()) showFieldError('username', '请输入学号或工号') }}
                  />
                </div>
              ...
              <div className="login-field" style={{ marginBottom: 20 }}>
                <label style={{ display: 'block', color: 'rgba(232,245,251,.76)', fontSize: 13, fontWeight: 600, marginBottom: 7 }}>密码</label>
              ...
              <div className="login-field">
                <button
                  ...
                  style={{
                    ...
                    background: success ? '#10b981' : styles.submit.background,
                  }}
                >
                  {loading ? <><span className="login-spinner" />登录中...</> : success ? '登录成功' : '登录'}
                </button>
              </div>
            </form>
            ...
            <p className="login-field" style={{ margin: '24px 0 0', textAlign: 'center', color: 'rgba(232,245,251,.46)', fontSize: 14 }}>
              还没有账号？
              <Link to="/register" style={{ color: '#a8d6ee', fontWeight: 700, textDecoration: 'none', marginLeft: 4 }}>立即注册</Link>
            </p>
          </section>
        </div>
      </main>
